...
export default class Detail extends PureComponent {
  constructor(props) {
    ...
    let initClassName = s.detail
    if (action === 'POP') {
      initClassName += ` ${s.animClassName}`
    }
    this.state = {
      animClassName: initClassName,
      data: {},
    }
  }
  ...
  componentDidMount() {
    window.setTimeout(() => {
      this.setState({ animClassName: `${s.detail} ${s.animClassName}` })
    }, 0)
  ...
  setClassName = () => {
    this.setState({
      animClassName: `${s.detail} ${s.animClassName} ${s.leaveClassName}`,
    })
  }

  render() {
    const { match } = this.props
    const { animClassName, data } = this.state
      ...
      <div>
        <div className={animClassName}>
          <div>
            ...
            <Link to={`${match.path}/iframe`} style={{ position: 'fixed', width: '100%', top: 0, padding: '1rem 0', textAlign: 'center', background: '#d33' }}>Click me!</Link>
          </div>
        </div>
        <div>
          <Route path={`${match.path}/iframe`} render={() => <Iframe setClassName={this.setClassName} />} />
        </div>
      </div>
    )
  }
